...
public class MinesweeperServer {
    ...
    //  The server creates MinesweeperRunnables to handle new client connections and 
    //  creates a MinesweeperGameRunnable to handle interactions with a new game.
    //  The server communicates with MinesweeperGameRunnable when there's an 
    //  incoming client connection by sending a RequestMessage over a linked blocking queue.
    //  This message contains a blocking queue, which MinesweeperGameRunnable uses to 
    //  communicate with that specific MinesweeperRunnable.
    //  MinesweeperRunnables send requests to interact with the game to MinesweeperGameRunnable 
    //  by sending a RequestMessage over a linked blocking queue with a specific id number.
    //  Then after processing a request, MinesweeperGameRunnable sends a reply to the MinesweeperRunnable
    //  that corresponds with the id number.
    ...
    /** Board the minesweeper game is played on. */
    ...
    public void serve() throws IOException {
        new Thread(new MinesweeperGameRunnable(board, debug, requests)).start();;
        while (true) {
            try {
                // block until a client connects
                BlockingQueue<ReplyMessage> ReplyQueue = new LinkedBlockingQueue<>();
                requests.put(new PlayerRequest(RequestMessage.Message.NEWPLAYER, ReplyQueue));
                new Thread(new MinesweeperRunnable(serverSocket.accept(), requests, ReplyQueue, client_id)).start();
                client_id++;
            }
